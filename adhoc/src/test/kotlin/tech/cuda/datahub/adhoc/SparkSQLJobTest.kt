 ...
 * @since 1.0.0
 */
class SparkSQLJobTest : AnnotationSpec() {

    private fun autoSetDerbyHome(block: (Map<String, String>) -> Unit) {
        val tempDir = createTempDir(prefix = "__adhoc__", suffix = ".derby")
        val derbyHome = tempDir.path.replace("\\", "/")
        block(mapOf(
    ...
    @Test
    fun testErrorStatement() = autoSetDerbyHome { sparkConf ->
        val job = SparkSQLJob(
            code = "select not_exists_column",
            sparkConf = sparkConf
        )
        job.startAndJoin()
    ...
    @Test
    fun testNotExistsTableStatement() = autoSetDerbyHome { sparkConf ->
        val job = SparkSQLJob(
            code = "select 1 from not_exists_table",
            sparkConf = sparkConf
        )
        job.startAndJoin()
        job.status shouldBe JobStatus.FAILED
    ...
    @Test
    fun testSelectStatement() = autoSetDerbyHome { sparkConf ->
        val job = SparkSQLJob(
            code = """
                select 
                    1 as int_column,
    ...
    @Test
    fun testUnionAllStatement() = autoSetDerbyHome { sparkConf ->
        val job = SparkSQLJob(
            code = """
    ...
    @Test
    fun testGroupByStatement() = autoSetDerbyHome { sparkConf ->
        val job = SparkSQLJob(
            code = """
                select
                    int_column,
    ...
    @Test
    fun testInnerJoinStatement() = autoSetDerbyHome { sparkConf ->
        val job = SparkSQLJob(
            code = """
                select
                    t1.int_column,
    ...
    @Test
    fun testLeftJoinStatement() = autoSetDerbyHome { sparkConf ->
        val job = SparkSQLJob(
            code = """
                select
                    t1.int_column,
    ...
    @Test
    fun testKillJob() = autoSetDerbyHome { sparkConf ->
        val job = SparkSQLJob(
            code = "select reflect('java.lang.Thread', 'sleep', bigint(30000))",
            sparkConf = sparkConf
        )
        job.start()
