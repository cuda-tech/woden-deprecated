...
object GroupService : Service(GroupDAO) {
    ...
    fun update(id: Int, name: String? = null): GroupDTO = Database.global.useTransaction {
        ...
        name?.let {
            GroupService.findByName(name)?.let { throw DuplicateException(I18N.group, name, I18N.existsAlready) }
            group.name = name
        }
        anyNotNull(name)?.let {
