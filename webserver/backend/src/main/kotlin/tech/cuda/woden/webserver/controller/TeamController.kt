...
import tech.cuda.woden.common.service.TeamService
...
class TeamController {
     ...
     * {"status":"success","data":{"teams":[{"id":2,"name":"testUpdate","createTime":"2029-05-26 23:17:01","updateTime":"2020-05-23 12:36:21"},{"id":3,"name":"cdqmxplc","createTime":"2045-06-15 10:48:04","updateTime":"2046-03-20 16:54:28"},{"id":4,"name":"rdiwafif","createTime":"2025-06-12 09:41:41","updateTime":"2027-01-04 14:36:46"}],"count":32}}
    ...
    fun listing(@RequestParam(required = false, defaultValue = "1") page: Int,
        ...
        val (teams, count) = TeamService.listing(page, pageSize, like)
        return Response.Success.data("teams" to teams, "count" to count)
    }

    /**
     * @api {get} /api/team/{id} 查找项目组
     * @apiDescription 查找指定 ID 的项目组信息，查找已删除的或不存在的项目组会返回失败
     * @apiGroup Team
     * @apiVersion 0.1.0
     * @apiHeader {String} token 用户授权 token
     * @apiSuccessExample 请求成功
     * {"status":"success","data":{"team":{"id":2,"name":"testUpdate","createTime":"2029-05-26 23:17:01","updateTime":"2020-05-23 12:36:21"}}}
     * @apiSuccessExample 请求失败
    ...
    fun find(@PathVariable id: Int): ResponseData {
        ...
        return if (team == null) {
            Response.Failed.WithError("${I18N.team} $id ${I18N.notExistsOrHasBeenRemove}")
        } else {
            Response.Success.data("team" to team)
        }
    }

    /**
     * @api {post} /api/team 创建项目组
     ...
     * @apiSuccessExample 请求成功
     * {"status":"success","data":{"team":{"id":40,"name":"testCreate","createTime":"2020-05-23 12:36:52","updateTime":"2020-05-23 12:36:52"}}}
     * @apiSuccessExample 请求失败
     ...
     * @apiSuccessExample 请求成功
     * {"status":"success","data":{"team":{"id":2,"name":"testUpdate","createTime":"2029-05-26 23:17:01","updateTime":"2020-05-23 12:36:20"}}}
     * @apiSuccessExample 请求失败
     * {"status":"failed","error":"项目组 1 不存在或已被删除"}
     */
    @PutMapping("{id}")
    fun update(@PathVariable id: Int, @RequestParam(required = false) name: String?) = try {
        Response.Success.data("team" to TeamService.update(id, name))
    } catch (e: NotFoundException) {
        Response.Failed.WithError(e.message ?: "系统异常")
    }

    /**
     * @api {delete} /api/team/{id} 删除项目组
    ...
    fun remove(@PathVariable id: Int) = try {
        ...
        Response.Success.message("${I18N.team} $id ${I18N.hasBeenRemove}")
    } catch (e: NotFoundException) {
        Response.Failed.WithError(e.message ?: "系统异常")
    }
